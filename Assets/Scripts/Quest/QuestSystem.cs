...
public class QuestSystem : MonoBehaviour
{
    ...
    {
        foreach (var quest in quests.ToArray()) // 도중에 바뀔수있어서 ToArray()로 사본을 만듬
            quest.ReceiveReport(category, target, successCount);
    }

    public bool ContainsInActiveQuest(Quest quest)
        => activeQuests.Any(x => x.CodeName == quest.CodeName);
    public bool ContainsInCompletedQuest(Quest quest)
        => completedQuests.Any(x => x.CodeName == quest.CodeName);
    public bool ContainsInActiveAchievements(Quest achievement)
        => activeAchievements.Any(x => x.CodeName == achievement.CodeName);
    public bool ContainsInCompletedAchievements(Quest achievement)
        => completedAchievements.Any(x => x.CodeName == achievement.CodeName);
